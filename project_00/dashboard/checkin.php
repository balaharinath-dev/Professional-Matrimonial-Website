...
$sql="SELECT * FROM `login` WHERE `sfid`=?;";
$stmt=mysqli_prepare($conn,$sql);
if(!$stmt){
    die("Statement preparation failed:".mysqli_error($conn));
}
mysqli_stmt_bind_param($stmt,"s",$websfid);
...
$sql = "SELECT `photo`,`photoType` FROM `login` WHERE `sfid`=?;";
$stmt = mysqli_prepare($conn, $sql);
if (!$stmt) {
    die("Statement preparation failed: " . mysqli_error($conn));
}
...
?>
<?php
$actwebusername=$_SESSION["logunamenew"];
$dbhostname = "localhost";
$dbusername = "root";
$dbpassword = "";
$dbname = "practise_login";
...
$actstmt = mysqli_prepare($conn, $sql);
if (!$actstmt) {
    die("Statement preparation failed: " . mysqli_error($conn));
}
mysqli_stmt_bind_param($actstmt, "s", $actwebusername);
if (mysqli_stmt_execute($actstmt)) {
    mysqli_stmt_store_result($actstmt);
    mysqli_stmt_bind_result($actstmt, $actrealcolphoto,$actrealcolphototype);
    if (mysqli_stmt_num_rows($actstmt) > 0) {
        mysqli_stmt_fetch($actstmt);
        $actbfrimageData = $actrealcolphoto;
        $actimageType = $actrealcolphototype;
        $actformattedimgdata=str_replace('-', '+', $actbfrimageData);
        if ($actbfrimageData) {
            $actphotoUrl = $actimageType.','.$actformattedimgdata;
        } else {
            echo "Invalid image format.";
        }
    } else {
...
mysqli_stmt_free_result($actstmt);
...
if(mysqli_stmt_execute($actstmt)){
    mysqli_stmt_store_result($actstmt);
    mysqli_stmt_bind_result($actstmt,$actcolusername, $actcolfirstname, $actcolmidname, $actcollastname);
    if(mysqli_stmt_num_rows($actstmt)>0){
        mysqli_stmt_fetch($actstmt);
        $actstoredusername = $actcolusername;
        $actstoredfirstname = $actcolfirstname;
        $actstoredlastname = $actcollastname; 
        $actstoredmidname = $actcolmidname;        
    }
}
else{
    die("Statement preparation failed:".mysqli_error($conn));
...
$actname;
if($actstoredmidname==""){
    $actname=$actstoredfirstname." ".$actstoredlastname;
}
else{
    $actname=$actstoredfirstname." ".$actstoredmidname." ".$actstoredlastname; 
}
mysqli_stmt_free_result($actstmt);
mysqli_stmt_close($actstmt);
mysqli_close($conn);
                ...
                <div class="offcanvas offcanvas-start s-canvas" data-bs-scroll="true" tabindex="-1" id="offcanvasWithBothOptions" aria-labelledby="offcanvasWithBothOptionsLabel">
                    <div class="offcanvas-header my-2 pb-0">
                        <div class="row m-0 d-flex align-items-center justify-content-start">
                            <div class="col-2 px-0">
                                <img src="<?php echo $actphotoUrl ?>" style="border-radius: 100%; border:1px solid rgba(0,0,0,0.5);" height="50px" width="50px" class="img-fluid" alt="...">
                            </div>
                            <div class="col-10 ps-2 pe-0">
                                <div class="row m-0">
                                    <div class="col-12 px-0">
                                        <h5 class="my-1"><b><?php echo $actname ?></b></h5>
                                    </div>
                                    <div class="col-12 px-0">
                                        <h6 class="mb-0 overflow-hidden"><?php echo $actstoredusername ?></h6>
                                    </div>
                                </div>                               
                            </div>                           
                        </div>
                    ...
                    <div class="offcanvas-body pt-0">                        
                        ...
                        <div class="list-group">
                            ...
                            <a onclick="goBack()" class="list-group-item list-group-item-action">Filtered results</a>
                        </div>
                        <hr class="my-3 mx-0">
                        <div class="list-group">
                            <div class="list-group-item list-group-item-action active">
                                <div class="col-2 px-0" style="height: 35px; width: 35px">
                                    <img src="<?php echo $photoUrl ?>" style="border-radius: 100%; border:1px solid rgba(255,255,255,0.75);" class="img-fluid" alt="...">
                                </div>
                                <div class="col-10 ps-2 pe-0">    
                                    <?php echo $name."'"; ?><span style="font-size:13px;">s profile</span>          
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="card" style="flex-grow:1">
                            <div class="row h-100 g-0">
                                <div class="col-sm-6 col-12">
                                    <img src="<?php echo $photoUrl ?>" class="img-fluid rounded-start" alt="...">
                                </div>
                                <div class="col-sm-6 col-12">
                                    <div class="card-body">
                                        ...
                                        <div class="row m-0">
                                            ...
                                            <div class="col-12 p-0"><p class="card-text mb-3" style="color: #ff1493;"><b>Interests:</b> 0</p></div>
                                        </div>  
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="col-12">
                                    <div class="card-body">
                                        ...
                                        <div class="row m-0">
                                            ...
                                            <div class="col-12 p-0 px-1"><p class="card-text mb-2"><b>Profession:</b> <?php echo $storedjob ?></div>
