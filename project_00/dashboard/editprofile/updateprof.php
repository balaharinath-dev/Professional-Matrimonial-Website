...
if($_SERVER["REQUEST_METHOD"]=="POST"){
    ...
    `firstname`=?, `midname`=?, `lastname`=?, `gender`=?, `bloodgroup`=?,
    `dob`=?, `age`=?, `martialstatus`=?, `religion`=?, `community`=?,
    `caste`=?, `mobcode`=?, `mobno`=?, `instaid`=?, `doorno`=?,
    `street`=?, `area`=?, `city`=?, `pincode`=?, `state`=?,
    `country`=?, `nationality`=?, `native`=?, `school`=?, `highestqualification`=?,
    `college`=?, `job`=?, `annualincome`=?, `fathersname`=?, `mothersname`=?,
    `siblingno`=?, `smoking`=?, `hobbies`=?, `dietarypref`=?, `partneragerange`=?,
    `partnerageend`=?, `partnereduucation`=?, `partnerjob`=?, `partneranincome`=?, `partnerincomeend`=?,
    `otherpref`=?, `horoscope`=?, `abouturself`=?, `photo`=?, `photoType`=?
    WHERE `username`=?";
